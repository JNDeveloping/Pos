    ...
    await controller.quickCreate(
      {
        ...
        branchId: null,
        roles: [],
        permissions: ['products.create', 'prices.update'],
        tokenVersion: 0,
      },
      {
...
describe('alta de producto', () => {
  it('encola automáticamente una etiqueta para la sucursal configurada', async () => {
    const tx = {
      company: { update: jest.fn().mockResolvedValue({ productSequence: 1 }) },
      product: { create: jest.fn().mockResolvedValue({ id: 'product', name: 'Yerba' }) },
      productBarcode: { create: jest.fn(), createMany: jest.fn() },
      branchProduct: { create: jest.fn() },
      labelPrintQueue: { create: jest.fn() },
    ...
    await controller.create(
      ...
      },
      {
        ...
        unitType: 'UNIT' as never,
        taxRate: '21',
        alternativeBarcodes: ['7790000000001', '7790000000002'],
        branchConfig: { branchId: '00000000-0000-4000-8000-000000000001', salePrice: '2500' },
      },
    );
    expect(tx.labelPrintQueue.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ productId: 'product', branchId: '00000000-0000-4000-8000-000000000001' }),
    });
