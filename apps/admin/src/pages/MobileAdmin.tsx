...
export function MobileAdmin({ me, branches, initialBranchId }: { me: Me; branches: Branch[]; initialBranchId?: string }) {
  ...
  const [message, setMessage] = useState(''), [priceMode, setPriceMode] = useState(false), [cameraActive, setCameraActive] = useState(false), [searching, setSearching] = useState(false), [creating, setCreating] = useState(false);
  ...
  async function createProduct(event: FormEvent<HTMLFormElement>) {
    ...
    if (!branchId || creating) { setMessage(branchId ? 'El producto ya se está guardando.' : 'Seleccioná una sucursal antes de guardar.'); return; }
    const form = new FormData(event.currentTarget); setCreating(true); setMessage('Guardando producto…');
    try {
      let imageUrl: string | undefined; const photo = form.get('photo');
  ...
  return <main className="mobile-admin">
    ...
    {view === 'create' && <section className="mobile-form"><h1>{barcode ? 'Producto no encontrado' : 'Crear producto rápido'}</h1><p>Sólo nombre, barcode y precio son obligatorios.</p>{hasPermission(me, 'products.create') && hasPermission(me, 'prices.update') ? <form onSubmit={createProduct}><label>Nombre *<input name="name" required autoFocus disabled={creating}/></label><label>Barcode *<input name="barcode" inputMode="numeric" pattern="[0-9]+" defaultValue={barcode} required disabled={creating}/></label><label>Precio *<input name="price" type="number" inputMode="decimal" min="0" step="0.01" required disabled={creating}/></label><details className="mobile-optional"><summary>Datos opcionales</summary><div><label>Categoría<select name="categoryId" disabled={creating}><option value="">Sin clasificar</option>{categories.filter((c) => !c.parentId).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>{hasPermission(me, 'costs.update') && <label>Costo<input name="cost" type="number" inputMode="decimal" min="0" step="0.01" disabled={creating}/></label>}<label>Stock inicial local<input name="stock" type="number" inputMode="decimal" min="0" step="0.001" disabled={creating}/></label>{suppliers.length > 0 && <label>Proveedor<select name="supplierId" disabled={creating}><option value="">Sin proveedor</option>{suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}<label>Foto<input name="photo" type="file" accept="image/*" capture="environment" disabled={creating}/></label></div></details><button type="submit" className="mobile-primary" disabled={creating || !branchId}>{creating ? 'Guardando producto…' : 'Crear y seguir escaneando'}</button></form> : <p>Tu rol necesita permisos para crear productos y modificar precios.</p>}</section>}
  </main>;
}
