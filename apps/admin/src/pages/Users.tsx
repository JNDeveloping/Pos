...
export function Users() {
  ...
  const load = () =>
    Promise.all([api<User[]>('/users'), api<Role[]>('/roles')])
      .then(([u, r]) => {
        setUsers(u);
        setRoles(r);
      })
  ...
  async function saveRoles(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!editing) return;
    const f = new FormData(e.currentTarget);
    try {
      await api(`/users/${editing.id}`, { method: 'PATCH', body: JSON.stringify({ roleIds: f.getAll('roleIds') }) });
    ...
    <div className="space-y-5">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4">
          <form className="card w-full max-w-lg p-5" onSubmit={saveRoles}>
            ...
            <button className="btn-primary w-full">Guardar roles</button>
          </form>
        </div>
      )}
    </div>
