...
type Terminal = { id: string; branchId: string; code: string; name: string; active: boolean; printerName?: string; cashSessions?: { id: string; cashier: { firstName: string; lastName: string } }[] };
type PaymentKind = 'CASH' | 'DEBIT' | 'CREDIT' | 'TRANSFER' | 'QR' | 'ACCOUNT' | 'OTHER';
type PaymentMethod = { id: string; code: string; name: string; kind: PaymentKind; active: boolean; requiresReference: boolean };
...
export function Settings() {
  ...
  const [terminals, setTerminals] = useState<Terminal[]>([]), [paymentMethods, setPaymentMethods] = useState<PaymentMethod[]>([]);
  ...
  useEffect(() => {
    ...
    void loadGroups();
    if (branchId) { void api<Terminal[]>('/terminals').then((rows) => setTerminals(rows.filter((row) => row.branchId === branchId))); void api<PaymentMethod[]>('/payment-methods').then(setPaymentMethods); }
  }, [branchId]);
  ...
  const createTerminal = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const terminal = await api<Terminal>('/terminals', { method: 'POST', body: JSON.stringify({ branchId, name: form.get('name'), code: form.get('code'), printerName: form.get('printerName') || undefined, active: true }) });
    setTerminals([...terminals, terminal]); event.currentTarget.reset(); setMessage('Terminal creada y disponible para abrir caja.');
  };
  ...
  const toggleTerminal = async (terminal: Terminal) => {
    await api(`/terminals/${terminal.id}`, { method: 'PATCH', body: JSON.stringify({ branchId: terminal.branchId, code: terminal.code, name: terminal.name, printerName: terminal.printerName, active: !terminal.active }) });
  ...
  const saveTerminal = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!editingTerminal) return;
    const updated = await api(`/terminals/${editingTerminal.id}`, { method: 'PATCH', body: JSON.stringify(editingTerminal) });
    setTerminals(terminals.map((item) => item.id === editingTerminal.id ? editingTerminal : item));
    setEditingTerminal(undefined); setMessage(updated ? 'Terminal actualizada.' : 'No se pudo actualizar la terminal.');
  };
  ...
  return <div className="space-y-5">
    ...
    <section className="card settings-panel">
      ...
      {tab === 'POS' && <div className="md:col-span-2 space-y-5">
        {!branchId ? <p>Seleccioná una sucursal para configurar el POS.</p> : <>
          <div className="grid gap-4 md:grid-cols-3"><label>Modo<select value={pos.mode} onChange={(e) => setPos({ ...pos, mode: e.target.value as PosSettings['mode'] })}><option value="touch">Táctil</option><option value="compact">Compacto</option></select></label><NumberField label="Columnas de botones" value={pos.favoriteColumns} set={(favoriteColumns) => setPos({ ...pos, favoriteColumns })}/><label className="check-field"><input type="checkbox" checked={pos.showStock} onChange={(e) => setPos({ ...pos, showStock: e.target.checked })}/>Mostrar stock</label><label className="check-field"><input type="checkbox" checked={pos.showBarcode} onChange={(e) => setPos({ ...pos, showBarcode: e.target.checked })}/>Mostrar barcode</label><label className="check-field"><input type="checkbox" checked={pos.showClock} onChange={(e) => setPos({ ...pos, showClock: e.target.checked })}/>Mostrar reloj</label><label className="check-field"><input type="checkbox" checked={pos.autoPrintTicket} onChange={(e) => setPos({ ...pos, autoPrintTicket: e.target.checked })}/>Abrir impresión del ticket al cobrar</label></div>
          ...
          <div className="grid gap-5 border-t pt-5 lg:grid-cols-2"><section><h2 className="text-xl font-bold">Terminales</h2><div className="mt-3 space-y-2">{terminals.map((terminal) => <div className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 p-3" key={terminal.id}><span className="flex-1"><b>{terminal.name}</b><small className="block">{terminal.code} · {terminal.printerName || 'Sin impresora asociada'}</small><small className="block">{terminal.cashSessions?.[0] ? `Caja abierta · ${terminal.cashSessions[0].cashier.firstName} ${terminal.cashSessions[0].cashier.lastName}` : 'Caja cerrada'}</small></span><button onClick={() => setEditingTerminal(terminal)}>Editar</button><button className="badge" disabled={Boolean(terminal.cashSessions?.length)} onClick={() => void toggleTerminal(terminal)}>{terminal.active ? 'Desactivar' : 'Activar'}</button></div>)}</div><form className="mt-3 grid gap-2 sm:grid-cols-3" onSubmit={createTerminal}><input name="name" required placeholder="Nombre: Caja 1"/><input name="code" required placeholder="Código: CAJA-1"/><input name="printerName" placeholder="Impresora opcional"/><button className="btn sm:col-span-3">Crear terminal</button></form></section><section><h2 className="text-xl font-bold">Medios de pago</h2><p className="text-sm text-slate-500">La naturaleza determina si el pago impacta efectivo, banco/QR o cuenta corriente.</p><div className="mt-3 space-y-2">{paymentMethods.map((method) => <div className="grid grid-cols-[1fr_auto] items-center gap-2 rounded-lg bg-slate-50 p-3" key={method.id}><span><b>{method.name}</b><small className="block">{method.code}{method.requiresReference ? ' · requiere referencia' : ''}</small></span><input type="checkbox" checked={method.active} onChange={(event) => void updatePayment(method, event.target.checked)}/><select className="col-span-2" value={method.kind} onChange={(event) => void updatePaymentKind(method, event.target.value as PaymentKind)}><option value="CASH">Efectivo en caja</option><option value="DEBIT">Débito</option><option value="CREDIT">Crédito</option><option value="TRANSFER">Transferencia</option><option value="QR">QR / billetera</option><option value="ACCOUNT">Cuenta corriente</option><option value="OTHER">Otro</option></select></div>)}</div></section></div>
        </>}
      </div>}
    ...
    {editingTerminal && <div className="modal-backdrop"><form className="modal-card max-w-xl" onSubmit={saveTerminal}><header className="flex justify-between"><div><h2 className="text-xl font-bold">Editar terminal</h2><p className="text-sm text-slate-500">La sucursal asignada se conserva y el código es único dentro de ella.</p></div><button type="button" onClick={() => setEditingTerminal(undefined)}><X/></button></header><label>Nombre<input required value={editingTerminal.name} onChange={(event) => setEditingTerminal({ ...editingTerminal, name: event.target.value })}/></label><label>Código<input required value={editingTerminal.code} onChange={(event) => setEditingTerminal({ ...editingTerminal, code: event.target.value.toUpperCase() })}/></label><label>Impresora opcional<input value={editingTerminal.printerName ?? ''} onChange={(event) => setEditingTerminal({ ...editingTerminal, printerName: event.target.value })}/></label><footer className="flex justify-end gap-2"><button type="button" className="btn-secondary" onClick={() => setEditingTerminal(undefined)}>Cancelar</button><button className="btn">Guardar terminal</button></footer></form></div>}
  </div>;
}
