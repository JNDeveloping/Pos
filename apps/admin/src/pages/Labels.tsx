...
export function Labels(){
  ...
  async function print(){if(!branchId)return;if(tab==='catalog')await api('/labels/generated',{method:'POST',body:JSON.stringify({branchId,productIds:selected,template})});window.print()}
  ...
  return <>
    ...
    <section className={`print-grid label-template-${template.toLowerCase()}`}>{chosen.map(item=>{const queued='product'in item,p=queued?item.product:item,price=queued?item.newPrice:(item as Product).branchConfigs.find(x=>x.branch.id===branchId)?.salePrice;return <article className="print-label" key={item.id}>{template.includes('OFFER')&&<strong className="offer">OFERTA</strong>}<h2>{p.name}</h2><b className="price">$ {Number(price??0).toLocaleString('es-AR')}</b><p>{p.barcodes[0]?.barcode}</p><small>{p.internalCode} · {new Date().toLocaleDateString('es-AR')}</small></article>})}</section>
  </>;
}
