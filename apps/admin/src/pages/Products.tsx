...
export function Products() {
  ...
  const load = () => api<Page>(`/products?search=${encodeURIComponent(search)}&page=${page}&limit=20`).then(setResult);
  useEffect(() => {
    void load();
  }, [page]);
  useEffect(() => {
    Promise.all([api<Ref[]>('/categories'), api<Ref[]>('/brands'), api<Ref[]>('/branches')]).then(([c, b, br]) => {
      setCategories(c);
      setBrands(b);
      setBranches(br);
    });
