...
describe('POS cart', () => {
  ...
  it('keeps shortcuts centralized', () => {
    expect(POS_SHORTCUTS.F4).toBe('PAY');
    expect(POS_SHORTCUTS.F8).toBe('CHECK_PRICE');
    expect(POS_SHORTCUTS.F9).toBe('PRICE');
  });
});
describe('POS payments', () => {
  it('calculates a mixed payment', () =>
    expect(
      paymentSummary(20000, [
        { amount: 10000, isCash: true },
        { amount: 10000, isCash: false },
      ]),
    ).toEqual({ remaining: 0, change: 0 }));
  it('calculates cash change', () =>
    expect(paymentSummary(8500, [{ amount: 8500, receivedAmount: 10000, isCash: true }])).toEqual({
      remaining: 0,
      change: 1500,
    }));
});
